...
#soil_photos.zip İndir
file = 'soil_photos.zip'
url = 'http://apmonitor.com/pds/uploads/Main/'+file
urllib.request.urlretrieve(url, file)

#soil_photos.zip arşivi dışarı çıkarılıp silinir
with zipfile.ZipFile(file, 'r') as zip_ref:
    zip_ref.extractall('./')
os.remove(file)

# Veri işleme araçlarını oluşturuyoruz
...
# Model değişkenini oluşturuyoruz
model = Sequential()
...
# num_conv_layers'a dayalı ek evrişim katmanları ekliyoruz
...
# tamamen bağlı "yoğun" katmanlar ekliyoruz
for _ in range(num_dense_layers):
    model.add(Dense(layer_size))
    model.add(Activation('relu'))

# çıktı katmanı ekliyoruz
model.add(Dense(3))
...
#Test ediyoruz
test_image_filepath = test_data_directory + r'/Gravel/0.jpg'
print(make_prediction(test_image_filepath))

percentage_photo = test_data_directory + r"/Sand/5.jpg"
